...
def test_array_in_planner_with_mixed_address_types(dev, ape_dai, ape_weth, ape_uniswap):
    ...
    test_cases = [
        {"name": "string addresses", "path": [str(ape_dai.address), str(ape_weth.address)]},
        {"name": "raw addresses", "path": [ape_dai.address, ape_weth.address]},
        {
    ...
    for i, test_case in enumerate(test_cases):
        ...
        # Add swap operation
        test_planner.add(uniswap_wei.swapExactTokensForTokens(amount, 0, test_case["path"], dev.address, deadline))
