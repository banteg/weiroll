...
@pytest.fixture
def test_account():
    """Return a test account for transactions"""
    return accounts.test_accounts[0]

@pytest.fixture
def dai_contract():
    """Return an Ape contract for DAI token on mainnet"""
    return ApeContract("0x6B175474E89094C44Da98b954EedeAC495271d0F")

@pytest.fixture
def weth_contract():
    """Return an Ape contract for WETH token on mainnet"""
    return ApeContract("0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2")

@pytest.fixture
def uniswap_router():
    """Return an Ape contract for Uniswap V2 Router on mainnet"""
    return ApeContract("0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D")


def test_array_in_planner_with_ape_contracts(test_account, dai_contract, weth_contract, uniswap_router):
    """Test using address arrays with Weiroll planner and Ape contracts"""
    # Debug info about contracts
    print(f"DAI address: {dai_contract.address}")
    print(f"WETH address: {weth_contract.address}")
    print(f"Uniswap Router address: {uniswap_router.address}")
    
    # Create Weiroll contract wrappers
    dai_wei = Contract.createContract(dai_contract)
    weth_wei = Contract.createContract(weth_contract)
    uniswap_wei = Contract.createContract(uniswap_router)
    ...
    # Fix #1: Convert contract objects to string addresses first
    path = [str(dai_contract.address), str(weth_contract.address)]
    print(f"Path array: {path}")
    print(f"Path types: {[type(x) for x in path]}")
    
    # Approve DAI for Uniswap Router
    planner.add(dai_wei.approve(uniswap_router.address, amount))
    ...
    weth_amount = planner.add(
        uniswap_wei.swapExactTokensForTokens(
            ...
            0,  # Min amount out
            path,  # Path array 
            test_account.address,  # Recipient
            deadline
        )
    )
    
    # Transfer received WETH back to user
    planner.add(weth_wei.transfer(test_account.address, weth_amount))
...
def test_array_in_planner_with_mixed_address_types(test_account, dai_contract, weth_contract, uniswap_router):
    ...
    test_cases = [
        ...
        {
            "name": "raw addresses",
            "path": [dai_contract.address, weth_contract.address]
        },
        {
    ...
    for i, test_case in enumerate(test_cases):
        ...
        # Approve DAI
        test_planner.add(dai_wei.approve(uniswap_router.address, amount))
