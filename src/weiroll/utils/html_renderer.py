...
def render_html(
    ...
    state_colors = [
        "#DA4453", "#3DAEE9", "#2ECC71", "#F67400", "#8E44AD", 
        "#1ABC9C", "#F39C12", "#7F8C8D", "#C0392B", "#1D99F3",
        "#1CDC9A", "#FDBC4B", "#9B59B6", "#16A085", "#E67E22", 
        "#95A5A6", "#ED1515", "#11D116", "#EDD400", "#BA79F1"
    ]
    ...
    for i, command in enumerate(commands):
            ...
            if isinstance(input_val, int) or (isinstance(input_val, str) and input_val.isdigit()):
                ...
                if isinstance(numeric_val, int) and numeric_val < 0:
                    if "command_type" in command and command["command_type"] == "SUBPLAN":
                        if numeric_val == -1:
                            html.append(f"<div data-command-idx='{i}'>  <span class='tree-branch'>{branch_char}</span> <span class='input-label'>{input_label}</span> <span class='subplan'>&lt;Subplan&gt;</span></div>")
                        else:
                            html.append(f"<div data-command-idx='{i}'>  <span class='tree-branch'>{branch_char}</span> <span class='input-label'>{input_label}</span> <span class='subplan'>&lt;Special Value: {numeric_val}&gt;</span></div>")
                    else:
                        html.append(f"<div data-command-idx='{i}'>  <span class='tree-branch'>{branch_char}</span> <span class='input-label'>{input_label}</span> <span class='state-ref'>&lt;Special Value: {numeric_val}&gt;</span></div>")
                ...
                # Regular state reference
                    ...
                    state_ref = f"<span class='{state_color_class}' data-state-ref='{numeric_val}' " + \
                    ...
                    if source_cmd >= 0:
                        ...
                        html.append(f"<div data-command-idx='{i}' data-state-used='{numeric_val}'>  <span class='tree-branch'>{branch_char}</span> <span class='input-label'>{input_label}</span> {state_ref} (from {cmd_ref} output)</div>")
                        ...
                        html.append(f"<div data-command-idx='{i}' data-state-used='{numeric_val}'>  <span class='tree-branch'>{branch_char}</span> <span class='input-label'>{input_label}</span> {state_ref} = <span class='{value_class}'>{value_formatted}</span></div>")
                    else:
                        # Reference to a state that will be computed
                        html.append(f"<div data-command-idx='{i}' data-state-used='{numeric_val}'>  <span class='tree-branch'>{branch_char}</span> <span class='input-label'>{input_label}</span> {state_ref}</div>")
            else:
                # Handle non-integer inputs
                from ..utils.formatters import format_value
                value_text = format_value(input_val)
                html.append(f"<div data-command-idx='{i}'>  <span class='tree-branch'>{branch_char}</span> <span class='input-label'>{input_label}</span> <span class='value-string'>{value_text}</span></div>")
        ...
        if outputs:
            for output_val in outputs:
                ...
                state_ref = f"<span class='state-{color_idx}' data-state-ref='{numeric_output_val}' " + \
                ...
                if usage_details:
                    if len(usage_details) == 1:
                        ...
                        if fn_name and param_name:
                            ...
                            html.append(f"<div data-command-idx='{i}' data-state-source='{numeric_output_val}'>  <span class='tree-branch'>└─</span> <span class='output-label'>{output_label}</span> {state_ref} <span class='arrow'>→</span> {cmd_ref} ({param_ref})</div>")
                        else:
                            cmd_ref = f"<span class='function-name'>Command {cmd_idx}</span>"
                            html.append(f"<div data-command-idx='{i}' data-state-source='{numeric_output_val}'>  <span class='tree-branch'>└─</span> <span class='output-label'>{output_label}</span> {state_ref} <span class='arrow'>→</span> {cmd_ref}</div>")
                    else:
                        # Multiple usages
                        usage_strs = []
                        ...
                        html.append(f"<div data-command-idx='{i}' data-state-source='{numeric_output_val}'>  <span class='tree-branch'>└─</span> <span class='output-label'>{output_label}</span> {state_ref} <span class='arrow'>→</span> " + ", ".join(usage_strs) + "</div>")
                else:
                    html.append(f"<div data-command-idx='{i}' data-state-source='{numeric_output_val}'>  <span class='tree-branch'>└─</span> <span class='output-label'>{output_label}</span> {state_ref} <span class='unused'>(unused in future commands)</span></div>")
